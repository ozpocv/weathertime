...
export default function WeatherPage() {
  ...
  const { suggestion, pending, activeChat, loading: compLoading, findCompanion, sendRequest, accept, decline, sendMessage, closeChat } = useCompanion(coords, !!user);
              ...
              <div key={req.id} className="pending-item">
                ...
                <div className="companion-actions" style={{ marginTop: '8px' }}>
                  <button className="comp-btn" onClick={() => {
                    const reqId = req._id?.toString() || req.id?.toString() || req.id;
                    accept(reqId, req.sender_username, req.activity);
                  }}>Accept</button>
                  <button className="comp-btn refuse" onClick={() => {
                    const reqId = req._id?.toString() || req.id?.toString() || req.id;
                    decline(reqId);
                  }}>Decline</button>
                </div>
                      ...
                      {suggestion && (
                        <div className="companion-card" style={{ marginTop: '14px' }}>
                          ...
                          <div className="companion-actions">
                            ...
                            <button className="comp-btn refuse" onClick={findCompanion}>Skip</button>
                          </div>
                        </div>
                      )}
