...
module.exports = {
  async getByCity(req,res,next)   { try { if(!req.query.city) return res.status(400).json({error:'city required'}); res.json(await W.getByCity(req.query.city)); } catch(e){next(e);} },
  ...
  async getQuote(req,res,next)    { try { res.json(await W.getQuote()); } catch(e){next(e);} },
};
